...
/*공통사항
 * 둘 다 반드시 메소드가 같은 이름일때만 고민한다.
 * 메소드 이름이 다른건 고려 대상이 아니다.
 * 
 * 메소드 오버로딩
 * 자바에서는 같은 이름의 메소드를 중복정의 할 수 있다.
 * 단 반드시 파라미터의 갯수나 타입이 달라야한다.
 * 리턴 타입이 있고 없고는 영향이 없다.
 * 접근 제한자가 있고 없고 나 범위가 다른 것도 영향이 없다.
 * 
 * 메소드 오버라이딩
 * 전제 조건으로 반드시 상속관계에 있어야된다.
